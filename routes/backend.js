    ...
    .then (async function(){
       for (let i = 1; i <= numOfPages; i++) {
        ...
        for (let j = 1; j <= 10 ; j++) {
          ...
          try{
            let element = tab.findElement(By.xpath("//button//span[text()='Next']"))
            element.click();
          }
          catch(err){
       ...
       for (let i = 1; i <= numOfPages; i++) {
        console.log("1")
          for (let j = start_from; j <= 10 ; j++) {
            ...
            console.log("1.", j);
            let connectionButton = await tab.findElement(By.xpath(connectionButtonXpath)).then(async found => {
              console.log("found person")
              ...
              console.log(listPeople);
              await tab.findElement(By.xpath(connectionButtonXpath)).click()
                .then(async function(){
                  let findTimeOutP = await tab.manage().setTimeouts({
                    implicit: 10000, // 10 seconds
                  })
